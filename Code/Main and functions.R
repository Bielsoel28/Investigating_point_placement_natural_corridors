...
raster_a <- rast("Rasters/Raster_A.tif")
...
raster_b <- rast("Rasters/Raster_B.tif")
...
raster_c <- rast("Rasters/Raster_C.tif")
...
# ---- Edge_1,6_km  ----
raster_un_sis <- rast("Rasters/Raster_un_sis_km.tif")
...
raster_dos <- rast("Rasters/Raster_dos_km.tif")
...
girona <- rast("Rasters/girona.tif") # Raster of the case of study
girona_bg <- rast("Rasters/girona_background.tif") # Background raster for overlapping areas
rius <- st_read("Rasters/cat_ETRS89_31N.shp") # Shapes of the rivers for cost barriers
costa <- st_read("Rasters/costa_girona_ETRS.shp") # Shapes of the cosatlines
...
jac <- st_read("Real_case/jaciments_girona.shp") # Sites
...
# Correlation test #
# Testing the data
cor_test <- cor(c_v_sites[ ,c(2,3,4,5)], method = "spearman") 

# Plotting and saving the data
tiff(file.path("Real_case","Correlation_plot_spearman.tiff"), width = 7*300, height = 4*300, res = 300) # Width and height in pixels
