...
# Load data
corridors <- rast("Real_case/Real_case_mosaic.tiff") # Natural corridors raster
jac <- st_read("Data/Vectors/punts_jaciments.shp")   # Archaeological sites

# Initialize data frame to store results
c_v_sites <- st_drop_geometry(jac[ ,1])
c_v_sites[ ,"Point value"] <- NA
c_v_sites[ ,"25% higher values (3km buffer)"] <- NA
c_v_sites[ ,"10% higher values (3km buffer)"] <- NA
c_v_sites[ ,"Mean value (3km buffer)"] <- NA
...
for (i in 1:nrow(jac)) {
  ...
  # Create 3 km buffer
  buffer <- st_buffer(pt, dist = 3000)
  
  # Crop and mask raster to buffer
  cropped_raster <- crop(corridors, buffer)
  masked_raster <- mask(cropped_raster, buffer)
  
  # Extract non-NA raster values within buffer
  vals <- values(masked_raster, na.rm = TRUE)
  
  # Store values
  c_v_sites[i, "Point value"] <- terra::extract(corridors, pt, ID = FALSE)[[1]]
  c_v_sites[i, "25% higher values (3km buffer)"] <- quantile(vals, 0.75, na.rm = TRUE)
  c_v_sites[i, "10% higher values (3km buffer)"] <- quantile(vals, 0.90, na.rm = TRUE)
  c_v_sites[i, "Mean value (3km buffer)"] <- mean(vals, na.rm = TRUE)
  
  print(paste("final",i))
}

write.csv(c_v_sites, file.path("Real_case", "Corridor_values_sites.csv"), row.names = FALSE)
...
# Save plot
ggsave(file.path("Real_case","Corridors_values_girona.tiff"), c_v_plot, width = 12, height = 8)
